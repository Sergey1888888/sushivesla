...
const Rolls: React.FC = () => {
    ...
    const {getRolls} = useActions();
    const {rolls} = useTypedSelector(state => state.rolls);
    ...
    return (<>
            {isMobile && <Container>
                <Text fontSize='17px' lineHeight='24px' fontWeight='bold' color='darkGrey'>Роллы</Text>
            </Container>
            }
            <Grid>
                {rolls.map(roll => <Item key={roll.id} image={roll.image} header={roll.name}
