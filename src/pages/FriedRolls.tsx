...
const FriedRolls: React.FC = () => {
    const {isMobile} = useContext<Partial<AdaptiveContextProps>>(AdaptiveContext);
    const {getFriedRolls} = useActions();
    const {friedRolls} = useTypedSelector(state => state.friedRolls);

    useEffect(() => {
        document.title = "СушиВесла - Жареные Роллы"
        getFriedRolls()
    }, [])

    return (<>
            {isMobile && <Container>
                <Text fontSize='17px' lineHeight='24px' fontWeight='bold' color='darkGrey'>Жареные Роллы</Text>
            </Container>
            }
            <Grid>
                {friedRolls.map(roll => <Item key={roll.id} image={roll.image} header={roll.name}
